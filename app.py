...
app.config['MYSQL_PASSWORD'] = 'Rahelu@123'
...
def generate_filename(userid, extension,class_name):
    return f"{userid}_{class_name}.jpg"
...
def signin():
    """
    This function render and returns the signin page for the users.
    
    This signup function checks whether the request method is POST or not.
    If it is a POST request, it saves the information from the form into the variable and 
    validate the credentials entered by the user with the credentials already stored in the database.
    Finally, it redirects the user to admin page if the user entered the admin login credentials 
    else it redirects to the homepage.
...
@app.route('/view_user_details/<user_id>')  
def view_user_details(user_id):
    """
    This function is used to view details of a specific user.

    Parameters
    ----------
    user_id : int
        The ID of the user whose details are to be viewed.

    Returns
    -------
    TYPE
        html : It returns the view_user_details.html file.
    """
    if 'loggedin' in session:
        if session['is_admin'] == 1:
            print('vieada')
            cursor = mysql.connection.cursor()
            cursor.execute("SELECT * FROM userdetails WHERE user_id = %s", (user_id,))
            user_details = cursor.fetchone()
            if user_details:
                return render_template('view_user_details.html', user_details=user_details)
            else:
    ...
    if 'loggedin' in session:
        if session['is_admin'] == 0:
            if request.method == 'POST':
                ...
                # Save uploaded files
                tenth_file = request.files['10th_certificate']
                twelveth_file = request.files['12th_certificate']
                
                # Check if files are images
                if tenth_file.filename != '' and allowed_file(tenth_file.filename):
                    tenth_filename = generate_filename(userid, tenth_file.filename.rsplit('.', 1)[1],class_name='tenth')
                    tenth_file.save(os.path.join(app.config['UPLOAD_FOLDER'], tenth_filename))
                if twelveth_file.filename != '' and allowed_file(twelveth_file.filename):
                    twelveth_filename = generate_filename(userid, twelveth_file.filename.rsplit('.', 1)[1],class_name='twelfth')
                    twelveth_file.save(os.path.join(app.config['UPLOAD_FOLDER'], twelveth_filename))
                ...
                if freeseats >0:
                    return redirect(url_for('seat_available'))
                else:
...
@app.route('/seat_available',methods = ['GET','POST'])
def seat_available():
    """
    ...
    if 'loggedin' in session:
        if session['is_admin'] == 0:
            if request.method == 'POST':
                ...
                email = session['email']
                # initializing global variables to use in other functions as well
                global app_no,department_id,department_name
                seat_locked = 1
...
def seat_not_available():
    ...
    if 'loggedin' in session:
        if session['is_admin'] == 0:
            render_template("seat_not_available.html")
    ...
    if 'loggedin' in session:
        if session['is_admin'] == 1:
            if request.method =='POST':
                ...
                seat_locked = int(request.form['seat_locked'])
                
                #update enquiry details in enquiry table
                cursor = mysql.connection.cursor()
                cursor.execute("select seat_locked from enquiry WHERE enquiry_no = %s",[enquiry_no,])
                enquiry_status = cursor.fetchone()[0]
                cursor.execute(" select sa.deptname,u.email from enquiry e join users u on e.user_id=u.userid join seat_availability sa on e.dept_id=sa.dept_id where e.user_id=%s",(user_id,))
                dept_name,user_email = cursor.fetchone()
                print(enquiry_status)
                if enquiry_status!=seat_locked:
                    cursor.execute(
                    ...
                    if seat_locked == 0:
                        print('resen')
                        sendmail(user_email, "Status of Enquiry", f"Dear Candidate,\n\nWe regret to inform you that your enquiry regarding the {dept_name} course has not been confirmed at this time. Please note that due to limited availability, we may not be able to accommodate all enquiries immediately. If you have any further questions or require assistance, please feel free to contact our support service.\n\nThank you for your interest in our programs.\n\nBest regards,\n[Guntur Engineering College]")
                    else:
                        print('senwre')
                        sendmail(user_email, "Status of Enquiry", f"Dear Candidate,\n\nWe are pleased to inform you that your enquiry regarding the {dept_name} course has been confirmed. A seat has been reserved for you in the upcoming intake. Please follow the instructions provided in the subsequent communication to complete your enrollment process.\n\nIf you have any further questions or require assistance, please feel free to contact our support service.\n\nWe look forward to welcoming you to our institution.\n\nBest regards,\n[Guntur Engineering College]")
